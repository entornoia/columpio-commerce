...
import { mockCollections } from "@/lib/storefront/mock-data";
    ...
    <header className="store-header">
      <div className="store-header-row">
        <details ref={mobileMenu} className="store-mobile-drawer store-mobile-only">
          ...
          <nav aria-label="Categorías">{mockCollections.map((item) => <Link key={item.slug} href={collectionPath(item.slug)} onClick={closeMobileMenu}>{item.name}</Link>)}</nav>
        </details>
        ...
        <nav className="store-desktop-nav" aria-label="Navegación principal">
          <Link href="/">Inicio</Link>{mockCollections.slice(0, 5).map((item) => <Link key={item.slug} href={collectionPath(item.slug)}>{item.name}</Link>)}
        </nav>
