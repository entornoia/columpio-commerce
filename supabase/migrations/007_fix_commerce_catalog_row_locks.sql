-- BLOQUE 4A/4B: mantener el catálogo read-only al serializar carrito y pedido.
...
begin
  ...
  if p_operation_type in ('add_to_cart', 'set_cart_quantity') then
    ...
    from public.product_variants v join public.products p on p.id=v.product_id where v.id=p_variant_id;
...
begin
  ...
  for item in select i.id, i.quantity, i.unit_price, p.price current_price, p.active product_active, v.active variant_active, v.stock
    ...
    join public.product_variants v on v.id=i.variant_id where i.cart_id=v_cart_id for update of i
  loop
